...
if(!toastr){
    toastr={error:function(err,title){
        ...
        window.top.wxbalert.notice({text:title+'<br>'+err,delay:4});
    }};
}
if(window.parent.wxbEditor && window.parent.wxbEditor.accountInfo){
    accountInfo=window.parent.wxbEditor.accountInfo;
}else{
    accountInfo=window.top.Accounts.getCurrentAccount();
}

var defaultData={"type":2,"file_cnt":{"total":0,"img_cnt":0,"voice_cnt":0,"video_cnt":0,"app_msg_cnt":0,"commondity_msg_cnt":0,"video_msg_cnt":0,"short_video_cnt":0,"app_msg_sent_cnt":0},"file_item":[],"file_group_list":{"biz_uin":0,"file_group":[{"id":1,"name":"未分组","files":[],"count":0}]},"watermark_status":3};
...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = 'http://127.0.0.1/wxb',//todo 这里 改了多文件上传的 逻辑， 客户端需求
                acceptExtensions = 'gif,jpg,jpeg,bmp,png',
                imageMaxSize = 5048000,
            ...
            uploader = _this.uploader = WebUploader.create({
                ...
                fileVal: "upfile",
                duplicate: true,
                fileSingleSizeLimit: imageMaxSize,    // 默认 5 M
                compress: false
            });
            uploader.addButton({
                id: '#filePickerBlock'
    ...
    OnlineImage.prototype = {
        init: function () {
            this.groupContainer=$('#onlineGroup');
            this.listContainer=$('#imageList');
            this.imgaePager=$('#imgaePager');
            this.reset();
            this.initEvents();
        ...
        initEvents: function () {
            ...
            container.on('click','#onlineGroup li',function(){
                ...
                _this.listTotalPage=Math.ceil(data_count/_this.listSize);
                _this.getImageData();
            });
            container.on('click','.js-pager-prev',function(){
                _this.listCurrentPage--;
                _this.getImageData();
            });
            container.on('click','.js-pager-next',function(){
                _this.listCurrentPage++;
                _this.getImageData();
            });
            container.on('click','.js-pager-go',function(){
                _this.listCurrentPage=container.find('.js-pager-num').val();
                _this.getImageData();
            });
        },
        /* 初始化第一次的数据 */
        initData: function () {
            ...
            /* 拉取数据需要使用的值 */
            this.group_id = 1;//分组ID
            this.listSize = 12;
            this.listCurrentPage = 1;
            this.listTotalPage = 1;
            /* 第一次拉取数据 */
            this.getImageData();
        },
        /* 重置界面 */
        ...
        showLoading:function(){
            $('#online .js-loading').show();
            this.isLoadingData = true;
        },
        hideLoading:function(){
            $('#online .js-loading').hide();
        ...
        getImageData: function () {
            //todo 这里已改为客户端 专用
            var _this = this;
            if(!_this.listCurrentPage<_this.listTotalPage && !this.isLoadingData) {
                _this.showLoading();
                WechatFetch.getImgList(accountInfo, (this.listCurrentPage-1)*this.listSize,this.group_id,this.listSize).then(function(response){
                    _this.render(response);
                    _this.hideLoading();
                }).catch(function (err) {
                    console.log(err);
                    toastr.error(err,"加载数据出错");
                    _this.hideLoading();
                });
        ...
        render:function(data){
            ...
            for (var i = 0; i < data.file_item.length; i++) {
                var obj = data.file_item[i];
                listHtml.push(`
                    <li><img src="${obj.cdn_url}" height="113"><span class="icon"></span></li>
                `);
            }

            for (var i = 0; i < data.file_group_list.file_group.length; i++) {
                var obj1 = data.file_group_list.file_group[i];
                if(obj1.id==_this.group_id){
                    groupListHtml.push(` <li group-id="${obj1.id}" data-count="${obj1.count}" title="${obj1.name}" class="on">${obj1.name} <span class="text-muted">（${obj1.count} ）</span></li> `);
                    _this.listTotalPage= Math.ceil(obj1.count/_this.listSize);
                }else{
                    groupListHtml.push(` <li group-id="${obj1.id}" data-count="${obj1.count}" title="${obj1.name}">${obj1.name} <span class="text-muted">（${obj1.count} ）</span></li> `);
                }

            }
            pagerHtml.push('<div class="material-pager">');
            if(_this.listCurrentPage>1){
                pagerHtml.push('<span class="js-pager-prev pager-btn" ><i class="fa fa-caret-left"></i></span>');
            }
            pagerHtml.push('<span class="js-pager-text pager-text"> '+_this.listCurrentPage+'/'+_this.listTotalPage+' </span>');
            if(_this.listCurrentPage<_this.listTotalPage){
                pagerHtml.push('<span class="js-pager-next pager-btn"><i class="fa fa-caret-right"></i></span>');
            }
            pagerHtml.push('<input class="js-pager-num pager-num" value="'+_this.listCurrentPage+'" type="text"/><span class="js-pager-go pager-btn">跳转</span></div>');

            this.listContainer.html(listHtml.join(''));
            this.groupContainer.html(groupListHtml.join(''));
            this.imgaePager.html(pagerHtml.join(''));
        ...
        getInsertList: function () {
            var list=[];
            var align = getAlign();
            this.listContainer.find('.selected').each(function () {
